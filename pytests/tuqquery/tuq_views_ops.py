...
class QueriesViewsTests(QuerySanityTests, QueryTests):
    ...
    def tearDown(self):
        try:
            self.cluster.delete_view(self.main, self.ddoc_name, None)
        except:
    ...
    def test_creating_views_query(self):
        ...
        tasks_view = self.async_create_views(self.main, self.ddoc_name, views)
        self.test_array_agg()
    ...
    def test_view_query(self):
        self.cluster.create_view(self.main, self.ddoc_name, self.default_view)
        self.test_array_agg()
        full_list = self.generate_full_docs_list(self.gens_load)
        task = self.cluster.async_query_view(self.main, self.ddoc_name,
    ...
    def test_view_query_simple(self):
        ...
        view_res = RestConnection(self.main).\
