...
class RbacN1QL(QueryTests):
    ...
    def test_select(self):
        ...
        cmd = "{4} -u {0}:{1} http://{2}:8093/query/service -d 'statement=SELECT * from {3} LIMIT 10'".\
                format(self.users[0]['id'], self.users[0]['password'], self.main.ip, self.buckets[0].name,self.curl_path)
    ...
    def test_update(self):
        ...
        cmd = "{6} -u {0}:{1} http://{2}:8093/query/service -d " \
              "'statement=UPDATE {3} a set name = '{4}' where name = '{5}' limit 1'".\
                format(self.users[0]['id'], self.users[0]['password'], self.main.ip, self.buckets[0].name, new_name, old_name,self.curl_path)
        output, error = shell.execute_command(cmd)
        shell.log_command_output(output, error)
    ...
    def test_insert(self):
        ...
        cmd = "%s -u %s:%s http://%s:8093/query/service -d " \
              "'statement=INSERT INTO %s (KEY, VALUE) VALUES(\"1\", { \"value1\": \"one1\" })'"%\
                (self.curl_path,self.users[0]['id'], self.users[0]['password'], self.main.ip, self.buckets[0].name)
        output, error = shell.execute_command(cmd)
        shell.log_command_output(output, error)
    ...
    def test_delete(self):
        ...
        cmd = "{5} -u {0}:{1} http://{2}:8093/query/service -d " \
              "'statement=DELETE FROM {3} a WHERE name = '{4}''".\
                format(self.users[0]['id'], self.users[0]['password'], self.main.ip, self.buckets[0].name, del_name,self.curl_path)
        output, error = shell.execute_command(cmd)
        shell.log_command_output(output, error)
    ...
    def test_merge(self):
        ...
        cmd = "%s -u %s:%s http://%s:8093/query/service -d " \
              ...
              "INSERT { \"value1\": \"one1\" }'"%\
                (self.curl_path,self.users[0]['id'], self.users[0]['password'], self.main.ip, self.buckets[0].name, self.buckets[0].name, 'name')
        output, error = shell.execute_command(cmd)
        shell.log_command_output(output, error)
        ...
        cmd = "%s -u %s:%s http://%s:8093/query/service -d " \
              ...
              "ON KEYS  b1.docid where b1.b1id > 1' " %\
                (self.curl_path,self.users[0]['id'], self.users[0]['password'], self.main.ip, self.buckets[0].name, self.buckets[0].name)
        output, error = shell.execute_command(cmd)
        shell.log_command_output(output, error)
    ...
    #             (self.curl_path,self.users[0]['id'], self.users[0]['password'], self.main.ip)
    #     output, error = shell.execute_command(cmd)
    #     shell.log_command_output(output, error)
    #
    ...
    def test_create_alter_index(self):
        ...
        cmd = "%s -u %s:%s http://%s:8093/query/service -d " \
              ...
              (self.curl_path, self.main.rest_username, self.main.rest_password,
               self.main.ip, self.buckets[0].name,self.main.ip+":"+self.main.port)
        output, error = shell.execute_command(cmd)
        shell.log_command_output(output, error)
        if "views_admin" in self.roles[0]['roles']:
        ...
        cmd = "%s -u %s:%s http://%s:8093/query/service -d " \
              ...
              (self.curl_path, self.users[0]['id'], self.users[0]['password'],
               self.main.ip, self.buckets[0].name,self.servers[1].ip+":"+self.servers[1].port)
        output, error = shell.execute_command(cmd)
        shell.log_command_output(output, error)
        ...
        for role in roles:
            ...
            if(assigned_role == 'admin'):
             cmd = "%s -u %s:%s http://%s:8093/query/service -d 'statement= SELECT * FROM %s limit 1'"\
                     %(self.curl_path,'test', 'password', self.main.ip,'default')
             output, error = shell.execute_command(cmd)
             shell.log_command_output(output, error)
    ...
    def test_prepare(self):
        ...
        if "delete" in self.roles[0]['roles'] :
            cmd = "{0} -u {1}:{2} http://{3}:8093/query/service -d 'statement=PREPARE delete from {4} LIMIT 10'".\
                format(self.curl_path,self.users[0]['id'], self.users[0]['password'], self.main.ip, self.buckets[0].name)
            output, error = shell.execute_command(cmd)
            shell.log_command_output(output, error)
    ...
    def test_infer(self):
        ...
        self._load_all_buckets(self.main, gen_load, "create", 10, flag=self.item_flag)
    ...
    def test_update_nested_with_select_with_full_access(self):
        ...
        cmd = "{6} -u {0}:{1} http://{2}:8093/query/service -d " \
              "'statement=UPDATE {3} a set name = '{4}' WHERE name IN (SELECT name FROM {5} WHERE age > 10)'".\
                format(self.users[0]['id'], self.users[0]['password'], self.main.ip, self.buckets[0].name, new_name, self.buckets[0].name,self.curl_path)
        output, error = shell.execute_command(cmd)
        shell.log_command_output(output, error)
        self.assertTrue(any("success" in line for line in output), "Unable to update from {0} as user {1}".
                        format(self.buckets[0].name, self.users[0]['id']))
    ...
    def test_delete_nested_with_select_with_full_access(self):
        ...
        cmd = "{5} -u {0}:{1} http://{2}:8093/query/service -d " \
              "'statement=DELETE FROM {3} a WHERE name IN (SELECT name FROM {4} WHERE age > 10)'".\
                format(self.users[0]['id'], self.users[0]['password'], self.main.ip, self.buckets[0].name, self.buckets[0].name,self.curl_path)
        output, error = shell.execute_command(cmd)
        shell.log_command_output(output, error)
        self.assertTrue(any("success" in line for line in output), "Unable to delete from {0} as user {1}".
                        format(self.buckets[0].name, self.users[0]['id']))
    ...
    def test_insert_nested_with_select_with_full_access_and_diff_buckets(self):
        ...
        cmd = "%s -u %s:%s http://%s:8093/query/service -d " \
              "'statement=INSERT INTO %s (KEY UUID(), VALUE _name)" \
              " SELECT _name FROM %s _name WHERE age > 10'"%\
                (self.curl_path,self.users[0]['id'], self.users[0]['password'], self.main.ip, self.buckets[1].name, self.buckets[0].name)
        output, error = shell.execute_command(cmd)
        shell.log_command_output(output, error)
    ...
    def test_update_nested_with_select_with_full_access_and_diff_buckets(self):
        ...
        cmd = "{6} -u {0}:{1} http://{2}:8093/query/service -d " \
              "'statement=UPDATE {3} a set name = '{4}' WHERE name IN (SELECT name FROM {5} WHERE age > 10)'".\
                format(self.users[0]['id'], self.users[0]['password'], self.main.ip, self.buckets[1].name, new_name, self.buckets[0].name,self.curl_path)
        output, error = shell.execute_command(cmd)
        shell.log_command_output(output, error)
    ...
    def test_delete_nested_with_select_with_full_access_and_diff_buckets(self):
        ...
        cmd = "{5} -u {0}:{1} http://{2}:8093/query/service -d " \
              "'statement=DELETE FROM {3} a WHERE name IN (SELECT name FROM {4} WHERE age > 10)'".\
                format(self.users[0]['id'], self.users[0]['password'], self.main.ip, self.buckets[1].name, self.buckets[0].name,self.curl_path)
        output, error = shell.execute_command(cmd)
        shell.log_command_output(output, error)
