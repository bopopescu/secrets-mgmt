...
class N1QLRBACTests(QueryTests):
    ...
    def suite_setUp(self):
        ...
        cmd = 'curl -X POST -u %s:%s -d name=bucket1 -d ramQuotaMB=100 -d authType=sasl -d password=pwd1' \
              ' http://%s:%s/pools/default/buckets'% (self.rest.username,self.rest.password,self.main.ip,self.port)
    ...
    def test_users_built_in_sasl_auth(self):
        ...
        cmd = 'curl http://%s:%s/settings/rbac/users -u %s:%s'  % (self.main.ip,self.port,self.rest.username,self.rest.password)
        o =self.shell.execute_command(cmd)
        ...
        cmd = 'curl -X PUT http://%s:%s/settings/rbac/users/builtin/user1 -d "name=User1&roles=data_reader_writer[bucket2]&' \
               'password=pwd1" -u %s:%s'%(self.main.ip,self.port,self.username,self.password)
        ...
        cmd = 'curl -X PUT http://%s:%s/settings/rbac/users/builtin/intuser -d "name=User2&roles=data_reader_writer[bucket2]' \
              '&password=pwd2" -u %s:%s'%(self.main.ip,self.port,self.username,self.password)
        self.shell.execute_command(cmd)
        #Query the list of users again
        cmd = 'curl http://%s:%s/settings/rbac/users -u %s:%s'%(self.main.ip,self.port,self.username,self.password)
        self.shell.execute_command(cmd)
        cmd = 'curl http://%s:%s/settings/rbac/users -u %s:%s'%(self.main.ip,self.port,'User1','pwd1')
        self.shell.execute_command(cmd)
        cmd = 'curl http://%s:%s/settings/rbac/users -u %s:%s'%(self.main.ip,self.port,'User2','pwd2')
        self.shell.execute_command(cmd)
    ...
    def test_users_crud_operation(self):
        ...
        cmd = 'curl -X PUT http://%s:%s/settings/rbac/users/builtin/intuser -d "name=TestInternalUser&roles=' \
              'data_reader[bucket2]&password=pwintuser" -u %s:%s'%(self.main.ip,self.port,self.username,self.password)
        self.shell.execute_command(cmd)
        #Read the internal user
        cmd = 'curl http://%s:%s/settings/rbac/users/builtin/intuser -u %s:%s'%(self.main.ip,self.port,self.username,self.password)
        o =self.shell.execute_command(cmd)
