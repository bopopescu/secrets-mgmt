...
class HostnameTests(BaseTestCase):
    ...
    def rest_api_addNode(self):
        ...
        main_rest.add_node(self.servers[1].rest_username, self.servers[1].rest_password, hostnames[1], self.servers[1].port)
    ...
    def rest_api_renameNode(self):
        try:
            ...
            if self.is_negative_test:
                main_rest = RestConnection(self.main)
                self.log.info("Renaming node {0} to {1}".format(self.main, self.name))
                var = main_rest.rename_node(username=self.main.rest_username, password=self.main.rest_password,
                    port=self.main.port, hostname=self.name, is_negative_test=True)
                out = var.pop()
                ...
                obj = RestConnection(self.main)
                var = obj.get_nodes_self().hostname
                flag = True if self.main.ip in var else False
                self.assertEqual(flag, False, msg="Fail - Node {0} is still referred via IP. Should\
                                     have been referred via hostname. Failing the test!".format(self.main.ip))
                self.log.info("Name of node {0} got converted to hostname. Proceeding......!".format(self.main.ip))
                self.sleep(10)
                self.log.info("Now changing name of node {0} from hostname to IP".format(self.main.ip))
                var = obj.rename_node(username='Administrator', password='password', port='', hostname=self.main.ip)
                self.log.info("Calling get_node_self() to check the status of the node {0}".format(self.main.ip))
                var = obj.get_nodes_self().hostname
                flag = True if self.main.ip in var else False
                self.assertEqual(flag, True, msg="Fail - Node {0} is still referred via hostname. Should have been referred via IP. Failing the test!".format(self.main.ip))
                self.log.info("Node {0} referred via IP. Pass !".format(self.main.ip))
        finally:
    ...
    def convert_to_hostname(self, servers_with_hostnames, username='Administrator', password='password'):
        try:
            ...
            for server in servers_with_hostnames:
                ...
                hostname.append(info.hostname[0] + "." + domain)
                main_rest = RestConnection(server)
                current_hostname = main_rest.get_nodes_self().hostname
                ...
                if server.ip in current_hostname:
                    ...
                    if version.startswith("1.8.1") or version.startswith("2.0.0") or version.startswith("2.0.1"):
                        ...
                        main_rest.init_cluster()
                    else:
                        main_rest.init_cluster()
                        main_rest.rename_node(username=username, password=password, port='', hostname=hostname[-1])
                else:
