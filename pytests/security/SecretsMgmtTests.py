...
class SecretsMgmtTests(BaseTestCase):
    ...
    def test_cli_setting(self):
        ...
        self.secretmgmt_base_obj.restart_server_with_env(self.main, self.password)
        temp_result = self.secretmgmt_base_obj.check_log_files(self.main, "/babysitter.log", "Initialization")
        self.assertTrue(temp_result, "Babysitter.log does not contain node initialization code")
...
class SecretsMgmtUpgrade(NewUpgradeBaseTest):
    ...
    def test_cluster_failover_graceful(self):
        servs_inout = self.servers[1:self.nodes_in + 1]
        self.cluster.failover(self.servers, servs_inout)
        self.cluster.rebalance(self.servers, [], [])
test_cluster_rebalance_in_env_var,services_init=kv-index-n1ql-kv
    '''
