...
class OpsDuringBackupTests(BackupBaseTest):
    ...
    def LoadDuringBackup(self):
        ...
        self._load_all_buckets(self.main, gen_load_backup, "create", 0, 2, self.item_flag, True, batch_size=20000, pause_secs=5, timeout_secs=180)
        ...
        data_load_thread = Thread(target=self._load_all_buckets,
                                  name="load_data",
                                  args=(self.main, gen_load, "create", 0, 1, 0, True))
        ...
        for bucket in self.buckets:
            del bucket.kvs[1]
        self._wait_for_stats_all_buckets(self.servers[:self.num_servers])
        self.verify_results(self.main, 2) #do verification only with kvstores[2]
    ...
    def CreateUpdateDeleteExpireDuringBackup(self):
        ...
        self._load_all_buckets(self.main, gen_load, "create", 0, 1, 0, True, batch_size=20000, pause_secs=5, timeout_secs=180)
        self._wait_for_stats_all_buckets(self.servers[:self.num_servers])

        mutate_threads = []
        if(self.doc_ops is not None):
            if("update" in self.doc_ops):
                mutate_threads.append(self._async_load_all_buckets(self.main, gen_update, "update", 0, 1, 0, True, batch_size=20000))

            if("delete" in self.doc_ops):
                mutate_threads.append(self._async_load_all_buckets(self.main, gen_delete, "delete", 0, 1, 0, True, batch_size=20000))

            if("expire" in self.doc_ops):
                mutate_threads.append(self._async_load_all_buckets(self.main, gen_expire, "update", self.expire_time, 1, 0, True, batch_size=20000))
