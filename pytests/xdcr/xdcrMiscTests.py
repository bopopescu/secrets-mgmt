...
class XdcrMiscTests(XDCRReplicationBaseTest):
    ...
    def test_verify_mb8825(self):
        ...
        self.merge_buckets(self.src_main, self.dest_main, bidirection=False)

        # Step-5 Wait for replication to finish 50% at destination node
        expected_items = (self.gen_create.end) * 0.5
        dest_main_buckets = self._get_cluster_buckets(self.dest_main)

        tasks = []
        for bucket in dest_main_buckets:
            tasks.append(self.cluster.async_wait_for_stats([self.dest_main], bucket, '', 'curr_items', '>=', expected_items))
        for task in tasks:
        ...
        self._replicate_clusters(self.dest_main, src_cluster_name)
        self.merge_buckets(self.dest_main, self.src_main, bidirection=False)
        ...
        src_main_buckets = self._get_cluster_buckets(self.src_main)
        for bucket in src_main_buckets:
    ...
    def test_diff_version_xdcr(self):
        ...
        # Step-3 Load 10k items ( sets=80, deletes=20) on source cluster.
        self._load_all_buckets(self.src_main, self.gen_create, "create", 0)
        self._load_all_buckets(self.dest_main, self.gen_create2, "create", 0)
    ...
    def test_xdcr_within_same_cluster(self):
        ...
        self._link_clusters(self.src_main, remote_cluster_name, self.src_main)
        ...
        self._load_bucket(src_bucket, self.src_main, self.gen_create, "create", 0)

        # Step-4 XDCR Source -> Remote
        rest_conn_src = RestConnection(self.src_main)
        ...
        self._verify_item_count(self.src_main, self.src_nodes)
        self._verify_data_all_buckets(self.src_main)
    ...
    def test_verify_mb15892(self):
        ...
        on source and destination mains
        """
        rest_conn_src = RestConnection(self.src_main)
        rest_conn_src.create_bucket(bucket='default', ramQuotaMB=256)
        main_id = rest_conn_src.get_nodes_self().id
        #if not cluster run use ip addresses instead of localhost
        if len(set([server.ip for server in self._servers])) != 1:
            main_id = main_id.replace("127.0.0.1", self.src_main.ip).replace("localhost", self.src_main.ip)
        self.buckets.append(Bucket(name="default", authType="sasl", saslPassword="",
                                       num_replicas=self._num_replicas, bucket_size=256, main_id=main_id,
                                       eviction_policy=self.eviction_policy))

        rest_conn_dest = RestConnection(self.dest_main)
        rest_conn_dest.create_bucket(bucket='default', ramQuotaMB=256, bucketType='memcached')
        main_id = rest_conn_dest.get_nodes_self().id
        #if not cluster run use ip addresses instead of localhost
        if len(set([server.ip for server in self._servers])) != 1:
            main_id = main_id.replace("127.0.0.1", self.dest_main.ip).replace("localhost", self.dest_main.ip)
        self.buckets.append(Bucket(name="default", authType="sasl", saslPassword="",
                                       num_replicas=self._num_replicas, bucket_size=256, main_id=main_id,
                                       eviction_policy=self.eviction_policy))

        remote_cluster_name = "C2"
        self._link_clusters(self.src_main, remote_cluster_name, self.dest_main)

        buckets = self._get_cluster_buckets(self.src_main)
        src_bucket = buckets[0]

        buckets = self._get_cluster_buckets(self.dest_main)
